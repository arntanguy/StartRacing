...
/**
 * Play the sounds for the vocal help. It plays only one sound at a time,
 * preventing sounds to overlap
 * 
...
public class VoiceRender extends AudioRender<String> {
	...
	public VoiceRender(Node rootNode, VoiceSoundStore soundStore) {
		super(rootNode, soundStore, false);
		playedNode = null;
	}

	/**
	 * Plays the voice corresponding to the given id, only if it is not already
	 * playing. If it is a different id than the one being played, stop the
	 * current playing, and play the new one.
	 * 
	 * @param sound
